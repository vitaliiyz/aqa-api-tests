...
def test_delete_user_success(created_user):
    ...
    assert (
        user_rs.status_code == 404
    ), f"The status code is not 404: {user_rs.status_code}\n{user_rs.text}"

    users_rs = get_users()
    assert (
        users_rs.status_code == 200
    ), f"The status code is not 404: {users_rs.status_code}\n{users_rs.text}"
    assert (
        created_user not in users_rs.json()
    ), f"User has not been removed: {users_rs.text}"
...
def test_delete_nonexistent_user_returns_404():
    ...
    """
    non_existent_user_test(delete_user, NON_EXISTING_USER_ID)
