...
class TestUsersFiltering:
    ...
    def test_filter_users_by_status(self):
        ...
        assert response.status_code == 200, f"The status code is not 200: {response.status_code}\n{response.text}"

        users = response.json()
        assert isinstance(users, list), "Response data should be a list"

        for user in users:
            assert user.get("status") == status_to_test, \
    ...
    def test_filter_users_by_name(self):
        ...
        for user in filtered_users:
            assert target_name.lower() in user.get("name", "").lower(), \
