...
def test_create_user_without_token_returns_401(user_data):
    ...
    # Pass empty headers dict to override default auth token
...
def test_delete_user_without_token_returns_401(created_user):
    """DELETE /users/{id} without token should return 401 Unauthorized"""
    response = delete_user(created_user["id"], headers={})

    assert response.status_code == 401, (
        f"Expected status code 401 when deleting without token, got {response.status_code}.\n"
        f"Response text: {response.text}"
    )


def test_update_user_without_token_returns_401(created_user):
    """PUT /users/{id} without token should return 401 Unauthorized"""
    update_payload = {"name": "Unauthorized Update"}

    response = update_user(created_user["id"], update_payload, headers={})

    assert response.status_code == 401, (
        f"Expected status code 401 when updating without token, got {response.status_code}.\n"
        f"Response text: {response.text}"
    )
...
def test_get_single_user_allowed_without_token(created_user):
    """Public read access: GET /users/{id} is accessible without token (200 OK)"""
    response = get_user_by_id(created_user["id"], headers={})

    assert response.status_code == 200, (
        f"Expected public GET /users/id to return 200 OK without token, got {response.status_code}.\n"
        f"Response text: {response.text}"
    )